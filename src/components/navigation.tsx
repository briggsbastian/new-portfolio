...
export default function Navigation({ className }: NavigationProps) {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 bg-black/70 backdrop-blur-md shadow-md ${className || ""}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex space-x-6">
            <Link href="/resume" className="text-white hover:text-indigo-400 transition-colors">
              Resume
            </Link>
            <Link href="/saint" className="text-white hover:text-indigo-400 transition-colors">
              Saint the Cat
            </Link>
          </div>
